...
Item {
    ...
    function refresh1(){
        ...
        for(var i = 0; i < list.length; i++){
            model1.append({"locationID": list[i].getLocationID,
                              "description": list[i].getDescription,
                              "latitude": list[i].getLatitude,
    ...
    function viewItem(){
        if(!stackView.busy)
            homeStackView.push("LocationItem.qml",{
                                   locationID: model1.get(view1.currentIndex).locationID,
                                   description: model1.get(view1.currentIndex).description,
                                   latitude: model1.get(view1.currentIndex).latitude,
                                   longitude: model1.get(view1.currentIndex).longitude
                               })
    }
    ...
    ListView {
        ...
        header: Row {
            ...
            Repeater {
                id: repeater1
                model: ["locationID", "description", "latitude", "longitude"]
                Label {
                    text: modelData
                    color: "#ffffffff"
                    font.pixelSize: 20
    ...
    Popup {
        ...
        TextField {
            ...
            clip: true
            selectByMouse: true
            placeholderText: "Enter the locationID"
        }
        ...
        Button {
            ...
            onClicked: {
                if(addField1.text===""||addField2.currentText===""||addField3.currentText===""||addField4.text==="")
                    addWarning1.visible = true
                else{
                    locationController.addLocation(addField1.text,addField2.text,addField3.text,addField4.text)
    ...
    Popup {
        ...
        Button {
            ...
            Material.foreground: "#FFFFFF"
            onClicked: {
                locationController.delLocation(model1.get(view1.currentIndex).locationID)
                deletePopup1.close()
                refresh1()
                overTimer.stop();
                if (subWindow.visible === true) return;
    ...
    function sort(Text){
        model1.clear()
        var list = locationController.sortLocation(Text)
        for(var i = 0; i < list.length; i++){
            model1.append({ "locationID": list[i].getLocationID,
                              "description": list[i].getDescription,
                              "latitude": list[i].getLatitude,
    ...
    function search(){
        if (searchField.text === ""){
            ...
            model1.clear()
            var list = locationController.searchLocation(sortBox.currentText, searchField.text)
            for(var i = 0; i < list.length; i++){
                model1.append({ "locationID": list[i].getLocationID,
                                  "description": list[i].getDescription,
                                  "latitude": list[i].getLatitude,
